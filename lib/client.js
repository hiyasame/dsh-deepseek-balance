window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		/** Selector of the strip `dsh-client-ui-chat` renders for conversation and token figures. */
		...
		*/
		function BalancePill({ t }) {
			...
			*/
			...
			react.useEffect(() => {
				if (typeof document === "undefined" || typeof document.querySelector !== "function") return void 0;
				/**
				* Track the statistics strip this pill renders inside.
				*
				* The strip is a `dsh-client-ui-chat` element with no child slot and
				* no exported component, so the pill is portaled into it: a portal
				* child becomes a real flex item of that row, inheriting its
				* centering and gap exactly like the shipped figures. The strip
				* arrives with the first completed step and leaves with the session,
				* hence the poll; until it exists the pill keeps its own row.
				*/
				const sync = () => {
					const found = document.querySelector(STATS_SELECTOR);
					if (found === hostRef.current) return;
			...
			if (host !== null && typeof react_dom.createPortal === "function") return react_dom.createPortal(anchor, host);
			return react.createElement("div", {
				className: css.root,
				"data-composer-balance-row": true
			}, anchor);
		}
		//#endregion
